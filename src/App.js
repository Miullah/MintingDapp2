...
function App() {
  ...
  const claimNFTs = (_amount) => {
    ...
    setFeedback("Minting your Fox Gang NFT...");
    setClaimingNft(true);
    blockchain.smartContract.methods
      .mint(blockchain.account, _amount)
      .send({
        gasLimit: "285000",
        to: "0x7ee8f4a8de978fcb78a02b62a17c7d3ceb498ecd",
      ...
      .then((receipt) => {
        setFeedback(
          "Good job, you just bought a Foxie."
        );
        setClaimingNft(false);
      ...
      <s.Container flex={1} ai={"center"} style={{ padding: 24 }}>
        ...
        <ResponsiveWrapper flex={1} style={{ padding: 24 }}>
          <s.Container flex={1} jc={"center"} ai={"center"}>
            <StyledImg alt={"left-image"} src={i1} />
            <s.SpacerMedium />
            <s.TextTitle
              style={{ textAlign: "center", fontSize: 35, fontWeight: "bold" }}
            >
              {data.totalSupply}/9950
            </s.TextTitle>
          </s.Container>
          <s.SpacerMedium />
          <s.Container
            ...
            jc={"center"}
            ai={"center"}
            style={{ backgroundColor: "#383838", padding: 25 }}
          >
            {Number(data.totalSupply) == 9950 ? (
              <>
                ...
                <s.TextDescription style={{ textAlign: "center" }}>
                  You can still find Fox Gang member on{" "}
                  <a
                    target={"_blank"}
                    href={"https://opensea.io/collection/foxgang"}
                  >
                    Opensea.io
                  </a>
                </s.TextDescription>
